...
void TransportManager::followPerimeter(BWAPI::Position to, BWAPI::Position from)
{
	...
	if (_transportShip->getDistance(from) < 50 && _waypoints.empty())
	{
		...
		F92_ASSERT_WARNING(valid, "waypoints not valid! (transport mgr)");
	...
	else if (_waypoints.size() > 1 && _transportShip->getDistance(_waypoints[0]) < 100)
	{
		...
		F92_ASSERT_WARNING(closestPolygonIndex != -1, "Couldn't find a closest vertex");
...
std::pair<int,int> TransportManager::findSafePath(BWAPI::Position to, BWAPI::Position from)
{
	...
	F92_ASSERT_WARNING(endPolygonIndex != -1, "Couldn't find a closest vertex");
	...
	F92_ASSERT_WARNING((_minCorner.isValid() && _maxCorner.isValid()), "Map corners should have been set! (transport mgr)");
...
{
	F92_ASSERT_WARNING(!_mapEdgeVertices.empty(), "We should have a transport route!");
