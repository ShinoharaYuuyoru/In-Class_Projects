...
void BOSSManager::startNewSearch(const std::vector<MetaPair> & goalUnits)
{
    ...
    // convert from F92's meta goal type to BOSS ActionType goal
    try
    {
...
void BOSSManager::update(double timeLimit)
{
    ...
    if (isSearchInProgress())
    {
        ...
        {
            F92_ASSERT_WARNING(false, "BOSS SmartSearch Exception: %s", exception.what());
        ...
        if (previousSearchComplete)
        {
            ...
            if (!solved && _previousBuildOrder.size() == 0)
            {
                ...
                {
                    F92_ASSERT_WARNING(false, "BOSS Timeout Naive Search Exception: %s", exception.what());
...
void BOSSManager::logBadSearch()
{
    ...
    Logger::LogOverwriteToFile("c:/F92error.txt", s);
}
...
BOSS::ActionType BOSSManager::GetActionType(const MetaType & t)
{
	...
	{
		F92_ASSERT(false, "Should have found a valid type here");
	}
