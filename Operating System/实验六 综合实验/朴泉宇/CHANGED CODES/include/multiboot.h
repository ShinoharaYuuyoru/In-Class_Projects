...
struct multiboot_t {
	 ...
	 * mmap_addr：缓冲区的地址，mmap_length：缓冲区的大小
...
typedef
struct mmap_entry_t {
	uint32_t size; 		// 此 size 是不含 size 自身变量的大小
	uint32_t base_addr_low;	// 低地址
	uint32_t base_addr_high;	// 高地址
	uint32_t length_low;	// 大小的低部分 为了防止32位表示不下
	uint32_t length_high;	// 大小的高部分
	uint32_t type;	// 内存段的属性 因为地址也有可能指向其他外设
