					...
					sprintf(sql,"update t_emailInfo set title=\"%s\",content=\"%s\" where type='2'",title,content);
						...
						if(!t)
						{
							...
							updateType();
                        		        ...
                        		        printf("success!\n");*/
						//printf("发送成功!\n");
						E_dialog=gtk_dialog_new_with_buttons("提示",GTK_WINDOW(E_window),GTK_DIALOG_MODAL,GTK_STOCK_OK,GTK_RESPONSE_OK,NULL);
						if(E_flag==7)
						{
				                	E_lbld=gtk_label_new("发送成功!");
						}
						else if(E_flag==2)
						{
							E_lbld=gtk_label_new("保存成功!");
						}
                				gtk_box_pack_start_defaults(GTK_BOX(GTK_DIALOG(E_dialog)->vbox),E_lbld);//标签加入到对话框里
                				gtk_widget_show_all(E_dialog);//显示对话框里的信息
                				if(gtk_dialog_run(GTK_DIALOG(E_dialog))==GTK_RESPONSE_OK)
                				{
                        				gtk_widget_destroy(E_dialog);//ro不是or
                				}
